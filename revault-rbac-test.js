...
const endpointMap = {
  ...
  edit_profile: {
    ...
    url: "/api/profile",
    requiresAuth: true,
    testData: { fullName: "Updated Name" },
  }, // Change from GET to PUT

  view_activity_logs: {
    method: "GET",
    url: "/admin/api/get-logs",
    requiresAuth: true,
  },
  generate_reports: {
    method: "GET",
    url: "/admin/api/activity-logs-report",
    requiresAuth: true,
  },
  view_admin_panel: {
    method: "GET",
...
async function makeRequest(endpoint, role, data = {}) {
  ...
  if (config.method !== "GET") {
    ...
    if (
      (endpoint === "login" || endpoint === "admin_login") &&
...
async function authenticateAllUsers() {
  ...
  for (const [role, userData] of Object.entries(CONFIG.testUsers)) {
    ...
    for (let attempt = 1; attempt <= CONFIG.retryAttempts; attempt++) {
      try {
        const loginEndpoint = userData.isAdmin
          ? "/admin/api/login"
          : "/api/login";

        console.log(
          `Attempting ${role} login via ${loginEndpoint} (attempt ${attempt})...`,
        );
        ...
        if (response.ok) {
          ...
          console.log(
            `✅ ${role}: Authenticated successfully via ${loginEndpoint}`,
          );
          authSuccess = true;
          break;
        } else {
          ...
          console.log(
            `❌ ${role}: Authentication failed (${response.status}) - ${errorText}`,
          );
...
async function calculateSuccessRate() {
  ...
  for (const role of roles) {
    ...
    for (const operation of operations) {
      ...
      console.log(
        `  ${operation}: ${result} Expected: ${test.expected}, Got: ${test.actual}${statusInfo}${categoryInfo}${errorInfo}`,
      );
...
function generateTestReport(
  ...
  roles.forEach((role) => {
    ...
    console.log(
      `${role}: ${roleRate}% (${roleCorrect}/${roleTests.length}) ${authStatus}`,
    );
  });
  ...
  const criticalIssues = detailedResults.filter(
    (test) =>
      !test.correct &&
      (test.expected === true ||
  ...
  if (criticalIssues.length > 0) {
    ...
    criticalIssues.forEach((issue) => {
      console.log(
        `  - ${issue.operation} for ${issue.role}: Expected ${issue.expected}, Got ${issue.actual} (${issue.category})`,
      );
    });
  }
...
async function checkEndpointHealth() {
  ...
  for (const [endpoint, config] of Object.entries(endpointMap)) {
    try {
      ...
      console.log(
        `${endpoint}: ${response.status >= 200 && response.status < 500 ? "✅" : "❌"} (${response.status})`,
      );
    } catch (error) {
      healthResults[endpoint] = {
        reachable: false,
...
function criticalSecurityTest() {
  ...
  criticalOps.forEach((operation) => {
    ...
    roles.forEach((role) => {
      ...
      const isSecure =
        (role === "ADMIN" &&
          ["manage_users", "create_user", "update_user"].includes(operation)) ||
