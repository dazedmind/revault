  ...
  return (
    <header className="flex flex-row align-middle bg-white z-50 items-center justify-between text-xl font-mono w-full p-6 px-8">
      ...
      <span className="flex items-center gap-4">
        ...
        <Link href={"../admin/login"} className="flex items-center gap-4 text-sm text-midnight hover:text-gold cursor-pointer group">
          <FaBuildingColumns className="text-2xl hover:text-gold cursor-pointer"/>
          <p className="hidden md:block text-sm">Sign in as Librarian</p>
          <FaChevronRight className="hidden md:block text-md hover:text-teal cursor-pointer opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300"/>
        </Link>
      </span>
      )}
    </header>
